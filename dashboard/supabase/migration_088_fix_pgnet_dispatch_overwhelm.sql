-- 277 timeouts in 10 minutes, 63% of them stalling on DNS resolution itself
-- for the full 5000ms -- not the edge function being slow, but our own
-- dispatch burst (30 requests, 0.5s apart) overwhelming pg_net's own worker
-- pool. Requests queue up waiting for a free worker and burn through the
-- entire timeout window before ever being sent. Result: the same submissions
-- get "dispatched" every single cron cycle for 50+ minutes and never
-- actually reach the edge function once.
-- Fix: slower stagger (less burst pressure) + longer pg_net timeout (more
-- room for a slow-but-working response to land before being written off).
