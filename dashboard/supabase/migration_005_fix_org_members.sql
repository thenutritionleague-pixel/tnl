-- ================================================================
-- Migration 005: Fix Org Members Sync
-- Backfills any missing profiles into `org_members` and updates the RPC 
-- so that new signups automatically join the organization.
-- ================================================================
...
-- 2. Update the RPC to include the org_members assignment 
...
begin
  ...
  -- 2. Essential Fix: Insert into org_members so Dashboard sees them!
  insert into public.org_members (org_id, user_id, role)
  values (p_org_id, v_profile_id, 'member')
  on conflict (org_id, user_id) do nothing;

  -- 3. Insert into team_members if team_id is provided
  ...
  -- 4. Mark the invite as used if invite_id is provided
